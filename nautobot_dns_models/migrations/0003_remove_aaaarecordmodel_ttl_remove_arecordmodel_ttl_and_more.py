# Generated by Django 4.2.21 on 2025-06-27 14:24

import django.core.validators
from django.db import migrations, models
import django.db.models.deletion


class Migration(migrations.Migration):

    dependencies = [
        ('ipam', '0051_added_optional_vrf_relationship_to_vdc'),
        ('nautobot_dns_models', '0002_srvrecordmodel'),
    ]

    operations = [
        migrations.RemoveField(
            model_name='aaaarecordmodel',
            name='ttl',
        ),
        migrations.RemoveField(
            model_name='arecordmodel',
            name='ttl',
        ),
        migrations.RemoveField(
            model_name='cnamerecordmodel',
            name='ttl',
        ),
        migrations.RemoveField(
            model_name='mxrecordmodel',
            name='ttl',
        ),
        migrations.RemoveField(
            model_name='nsrecordmodel',
            name='ttl',
        ),
        migrations.RemoveField(
            model_name='ptrrecordmodel',
            name='ttl',
        ),
        migrations.RemoveField(
            model_name='srvrecordmodel',
            name='ttl',
        ),
        migrations.RemoveField(
            model_name='txtrecordmodel',
            name='ttl',
        ),
        migrations.AddField(
            model_name='aaaarecordmodel',
            name='_ttl',
            field=models.IntegerField(blank=True, null=True, validators=[django.core.validators.MinValueValidator(300), django.core.validators.MaxValueValidator(2147483647)]),
        ),
        migrations.AddField(
            model_name='arecordmodel',
            name='_ttl',
            field=models.IntegerField(blank=True, null=True, validators=[django.core.validators.MinValueValidator(300), django.core.validators.MaxValueValidator(2147483647)]),
        ),
        migrations.AddField(
            model_name='cnamerecordmodel',
            name='_ttl',
            field=models.IntegerField(blank=True, null=True, validators=[django.core.validators.MinValueValidator(300), django.core.validators.MaxValueValidator(2147483647)]),
        ),
        migrations.AddField(
            model_name='mxrecordmodel',
            name='_ttl',
            field=models.IntegerField(blank=True, null=True, validators=[django.core.validators.MinValueValidator(300), django.core.validators.MaxValueValidator(2147483647)]),
        ),
        migrations.AddField(
            model_name='nsrecordmodel',
            name='_ttl',
            field=models.IntegerField(blank=True, null=True, validators=[django.core.validators.MinValueValidator(300), django.core.validators.MaxValueValidator(2147483647)]),
        ),
        migrations.AddField(
            model_name='ptrrecordmodel',
            name='_ttl',
            field=models.IntegerField(blank=True, null=True, validators=[django.core.validators.MinValueValidator(300), django.core.validators.MaxValueValidator(2147483647)]),
        ),
        migrations.AddField(
            model_name='srvrecordmodel',
            name='_ttl',
            field=models.IntegerField(blank=True, null=True, validators=[django.core.validators.MinValueValidator(300), django.core.validators.MaxValueValidator(2147483647)]),
        ),
        migrations.AddField(
            model_name='txtrecordmodel',
            name='_ttl',
            field=models.IntegerField(blank=True, null=True, validators=[django.core.validators.MinValueValidator(300), django.core.validators.MaxValueValidator(2147483647)]),
        ),
        migrations.AlterField(
            model_name='aaaarecordmodel',
            name='address',
            field=models.ForeignKey(limit_choices_to={'ip_version': 6}, on_delete=django.db.models.deletion.CASCADE, to='ipam.ipaddress'),
        ),
        migrations.AlterField(
            model_name='arecordmodel',
            name='address',
            field=models.ForeignKey(limit_choices_to={'ip_version': 4}, on_delete=django.db.models.deletion.CASCADE, to='ipam.ipaddress'),
        ),
    ]
